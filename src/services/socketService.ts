...
export function initSocket(token: string): CustomSocket {
  ...
  // Si un socket existe mais n'est pas connecté, tenter de le reconnecter avant de le remplacer
  if (socket) {
    try {
      socket.connect();
  ...
  socket = io(baseUrl, {
    ...
    reconnection: true,
    reconnectionAttempts: 20,  // Augmenter davantage
...
export function sendMessage(channelId: string, content: string, files?: any[]): void {
  if (socket && socket.connected) {
    socket.emit('send-message', { channelId, content, files });
  } else {
