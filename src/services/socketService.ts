...
export function setupSocketLifecycleManagement(token: string): () => void {
  ...
  const handleVisibilityChange = () => {
    ...
    if (document.visibilityState === 'visible') {
      // Page redevient visible - vérifier la connexion après un délai
      setTimeout(() => {
        if (!isIntentionalDisconnect && (!socket || !socket.connected)) {
          console.log('Page visible, reconnexion du socket...');
  ...
  document.addEventListener('visibilitychange', () => {
    if (document.hidden) {
      // Page cachée - ne pas déconnecter mais réduire l'activité
      console.log('Page cachée, maintien de la connexion socket');
    } else {
      ...
      setTimeout(() => {
        if (socket && !socket.connected) {
          const token = localStorage.getItem('token') || sessionStorage.getItem('token');
          if (token) {
            console.log('Page redevenue visible, reconnexion du socket');
            socket.connect();
          }
        }
      }, 500);
