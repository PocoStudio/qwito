...
function Login() {
  ...
  const handleGoogleLogin = () => {
    ...
    setTimeout(() => {
      if (isMobile) {
        // Sur mobile, forcer l'ouverture dans le navigateur externe
        // On utilise window.open avec les bonnes options pour forcer un nouvel onglet
        const newWindow = window.open(googleAuthUrl, '_blank', 'noopener,noreferrer');
        
        // Si window.open échoue (bloqueur de popup), fallback vers location.href
        if (!newWindow) {
          window.location.href = googleAuthUrl;
        } else {
          // Écouter les messages de la nouvelle fenêtre pour récupérer le token
          const messageHandler = (event: MessageEvent) => {
            ...
            if (event.data.type === 'GOOGLE_AUTH_SUCCESS' && event.data.token) {
              ...
              navigate('/panel');
              // Fermer la fenêtre popup si elle est encore ouverte
              if (newWindow && !newWindow.closed) {
                newWindow.close();
              }
            } else if (event.data.type === 'GOOGLE_AUTH_ERROR') {
              // Gérer l'erreur
              setMessage({ text: 'Erreur lors de l\'authentification Google', type: 'error' });
